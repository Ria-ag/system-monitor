...
if button_1:
    ...
    img = Image.open('utilities/logo.png')
    st.image(img)
    st.title('Welcome to our Resume Agit remote add origin https://github.com/Ria-ag/system-monitor.gitnalyzer!')
    st.subheader('Features')
    st.text('Performance Metrics: Comprehensive quality and relevance evaluation of entire resumes.')
    st.text('Anomaly Detection: Pinpoint inconsistencies, backed by an F1 score visualization.')
    st.text('Covariate Shift: Witness the evolution of data over time through intuitive bar graphs.')
    st.text('Univariate Shift: Keep an eye on trending resume keywords.')
else:
    img = Image.open('utilities/logo.png')
    st.image(img)
    st.title('Welcome to our Resume Agit remote add origin https://github.com/Ria-ag/system-monitor.gitnalyzer!')
